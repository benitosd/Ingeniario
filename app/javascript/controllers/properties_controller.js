...
export default class extends Controller {
  ...
  remove(event) {
    ...
    if (keyInput) {
      ...
      hiddenInput.name = "group[properties_to_remove][]";
  ...
  undoRemove(event) {
    ...
    // Desmarcar la fila como pendiente de eliminación
    row.classList.remove("marked-for-deletion");
    row.querySelectorAll("input").forEach((input) => (input.disabled = false));
  
    // Eliminar el mensaje visual
    ...
    const hiddenInput = this.containerTarget.querySelector(
      `input[name='group[properties_to_remove][]'][value='${keyInput.value}']`
    );
